        ...
        self.context.zk.a_get_children(CACHE_SERVER_MAPPING+'/'+e, false, function(rc, err, path) {
          ...
          dns.reverse(ip, function(err, domains) {
            if (err) {
              ...
              _.defaults(static_map[path], {"servers":{}, "hubble":hubble[path], "port":port});
              if (process.env.COMPOSE_NETWORK_NAME && domains[0].lastIndexOf(process.env.COMPOSE_NETWORK_NAME) > -1) {
                  // FIXME: if the orbiter runs in docker-compose, then remove the suffix project name.
                  // Robust dns driver could solve this problem.
                  domains[0] = domains[0].substr(0, domains[0].lastIndexOf('.' + process.env.COMPOSE_NETWORK_NAME));
              }
              // if there's an service code with a port, it should be a private cloud.
